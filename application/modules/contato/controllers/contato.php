...
class Contato extends MY_Controller
{
  ...
  public function send()
  {
    ...
    if ($this->form_validation->run()) {
      ...
      $id = $this->contato_m->insert_contact($data);
      ...
      $emails = array(
        'to' => ENVIRONMENT == 'development' ? 'foxittybr@gmail.com' : 'foxittybr@gmail.com',
        'replyTo' => $data['email']
      );
      ...
      if ($id) {
        if (enviar_email($emails, 'Soulx - Contato', $body)) {
          $this->contato_m->update_contact($id, array('status' => 'Não lida'));
        } else {
          $this->contato_m->update_contact($id, array('status' => 'Não enviada'));
        }
      }
    } else {
