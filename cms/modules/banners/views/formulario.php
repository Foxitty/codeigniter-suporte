<form action="<?= site_url($current_module->slug . '/' . (isset($item) ? 'edit/' . $id : 'add')) ?>"
    id="validateSubmitForm" class="form-horizontal" role="form" enctype="multipart/form-data" method="post">
    <?php if (isset($item)) { ?>
    <input type="hidden" name="id" value="<?= $id; ?>" id="inputId" />
    <?php   } ?>
    <!-- Nav tabs -->
    <ul class="nav nav-tabs col-sm-12">
        <li class="active"><a href="#tab1" data-toggle="tab"><i class="fa fa-file-alt"></i>
                <?= T_('Dados Gerais'); ?></a></li>
    </ul>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane<?= ($key == 0) ? ' active in ' : ''; ?> fade" id="tablang<?= $key; ?>">
                                <div class="col-xs-12 col-md-12 col-sm-12 form-group">
                                    <label for="inputTitle<?= $key; ?>" class="control-label"><?= T_('Título'); ?>:
                                    </label>
                                    <textarea type="text" class="form-control inputWithCK"
                                        name="value[<?= $language->id; ?>][title]" id="inputTitle<?= $key; ?>"
                                        placeholder="<?= T_('Título'); ?>"><?= (isset($item->languages[$language->id])) ? $item->languages[$language->id]->title : ''; ?></textarea>
                                </div>

                                <div class="col-xs-12 col-md-12 col-sm-12 form-group">
                                    <label for="inputSubtitle<?= $key; ?>"
                                        class="control-label"><?= T_('Subtítulo'); ?>: </label>
                                    <input type="text" class="form-control"
                                        name="value[<?= $language->id; ?>][subtitle]" id="inputSubtitle<?= $key; ?>"
                                        placeholder="<?= T_('Subtítulo'); ?>"
                                        value="<?= (isset($item->languages[$language->id])) ? $item->languages[$language->id]->subtitle : ''; ?>" />
                                </div>
